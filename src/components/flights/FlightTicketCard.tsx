...
export const FlightTicketCard: React.FC<FlightTicketCardProps> = ({
    ...
    // ---- Real PDF417 barcode for boarding pass (canvas-based) ----
    const barcodeValue = `${ticket.flightNumber || ""}|${
        ticket.bookingRef || ""
    }|${ticket.passenger || ""}`;
    ...
    useEffect(() => {
        ...
        const logicalWidth = 420;  // must match Tailwind w-[320px]
        const logicalHeight = 80;  // must match Tailwind h-[64px]
        const dpr = window.devicePixelRatio || 1;

        // Set internal pixel size = logical * DPR (for crispness)
        ...
        // Draw in logical coordinates, let DPR handle density
        ...
        // Let pdf417-generator draw at 1× DPR (we already scaled)
        try {
            PDF417.draw(
                barcodeValue,
                canvas,
                ...
                1    // devicePixelRatio (we're handling it ourselves)
            );
        } catch (err) {
            console.error("Failed to draw PDF417 barcode", err);
        }
    }, [barcodeValue]);
    ...
    return (
        ...
        >
            <div className="group relative [perspective:1400px]">
                <div
                    className={`relative h-[560px] sm:h-[640px] transition-transform duration-700 [transform-style:preserve-3d] ease-[cubic-bezier(0.22,1,0.36,1)] ${
                                    ...
                                    <div className="flex items-center gap-3">
                                        {airlineLogo1x ? (
                                            <img
                                                ...
                                                alt={
                                                    ticket.airline || airlineCode || "Airline logo"
                                                }
                                            />
                                        ) : (
                    ...
                    <div className="absolute inset-0 [backface-visibility:hidden] [transform:rotateY(180deg)]">
                        <div className="h-full rounded-[30px] bg-card text-foreground border border-border px-4 sm:px-6 pt-4 sm:pt-5 pb-4 flex flex-col shadow-xl overflow-hidden">
                            <div className="mb-3 sm:mb-4 border-b border-dashed border-border pb-3">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <div className="mt-0.5 text-base sm:text-lg font-bold tracking-tight text-foreground">
                                            {ticket.airline}
                                        </div>
                                    </div>
                                    {airlineLogo1x && (
                            ...
                            <div className="mb-3 sm:mb-4 space-y-2">
                                <div className="flex items-start gap-2">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        {fromAirport?.country &&
                                            typeof fromAirport.country === "string" && (
                            ...
                            {fromAirport.country}
                                    ...
                                    <div className="text-xs sm:text-sm font-medium text-foreground flex-shrink-0">
                                        {ticket.departureTime}
                                    </div>
                                </div>
                                ...
                                <div className="flex items-start gap-2">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        {toAirport?.country &&
                                            typeof toAirport.country === "string" && (
                                                <div className="mt-0.5 text-[10px] sm:text-xs text-muted-foreground flex items-center gap-1">
                                                    {toFlag && (
                                                        <img
                                                            src={toFlag}
                            ...
                            <div className="flex items-center gap-2 border-t border-dashed border-border pt-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="mt-0.5 text-sm sm:text-base font-bold tracking-[0.15em] text-foreground">
                                        {ticket.bookingRef}
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* REAL PDF417 BARCODE */}
                            <div className="mt-3 sm:mt-4 rounded-lg bg-muted/10 p-2.5 sm:p-3 text-center">
                                ...
                                <div className="flex justify-center">
                                    <canvas
                                        ...
                                        className="w-[420px] h-[80px] bg-white rounded shadow-sm"
                                    />
                                </div>
                            </div>

                            <div className="mt-2 sm:mt-3 text-center text-[10px] sm:text-xs text-muted-foreground/50">
                                Click to flip back
                            </div>
                        </div>
                    </div>
